...
```
sudo dnf upgrade -y
sudo dnf install curl git unzip clang xz zip mesa-libGLU ninja-build gtk3-devel cmake libsecret-devel jsoncpp-devel
...
Edit `linux/CMakeLists.txt` and add the following line after `set(APPLICATION_ID "com.example.src")`:
